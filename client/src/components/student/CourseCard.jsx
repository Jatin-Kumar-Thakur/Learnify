    ...
    <Link to={'/course/' + props?._id} onClick={() => scrollTo(0, 0)} className='border rounded-lg overflow-hidden cursor-pointer'>
      ...
      <div className='text-left p-3'>
        ...
        <div className="flex items-center space-x-2">
          <p>{averageRating(props?.courseRating)}</p>
          <div className="flex items-center ml-2 text-sm">
            {
              [...Array(5)].map((_, index) => (
                <span key={index} className={`text-yellow-500 ${index < 4 ? 'text-yellow-500' : 'text-gray-300'}`}>
                  <img src={index < Math.floor(averageRating(props?.courseRating)) ? assets.star : assets.star_blank} alt="star" className=' w-[14px]' />
                </span>
              ))
            }
          </div>
          <p className='pl-2 text-gray-500 '>{props?.courseRating?.length}</p>
        </div>
